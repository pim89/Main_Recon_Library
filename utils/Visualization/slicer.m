...
% - Enable scrolling through time-series of 4D dataset, including updates
%   of all slices
% - Disabled drag function (use scrolling in subplot 4)
...
%                 set(f1, 'WindowButtonMotionFcn', @dragCallback);
                ...
                set(f1, 'WindowButtonUpFcn', @click);
%                 set(f1, 'WindowButtonUpFcn', @stopdragAndClick);
%                 set(f1, 'WindowButtonUpFcn', @stopdrag);
            ...
            elseif strcmp(s,'alt')
                ...
                crossPlot = whichView; %save which plot the crosshair is in
                for crossAxis=1:3
...
%                     crossLoc=[p(3),p(1),k(3)];
                    ...
                    upDateSliceOtherPlanes(p(3),2);
                    upDateSliceOtherPlanes(p(1),3);
...
%% Scroll function
    function scroller(~,evnt)
        ...
        % Hide crosshair if scrolling in other plot
        if crossPlot ~= whichView && crossPlot ~= 0 && whichView ~= 4
            for crossAxis = 1:3
                set(crossHandle{crossAxis}, 'Color',[1,0,0,0]); %transparency: 100%
            end
        end
...
%% Combined function of stopdrag and click
% % % % %     function stopdragAndClick(~,~)
% % % % % %         stopdrag; %disabled for now
% % % % %         click;
% % % % %     end
