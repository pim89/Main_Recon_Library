function [dvf, f_img] = optical_flow_mrriddle(data,alpha,ref,varargin)
% Register the reference to the next image, instead of the other way around
...
for dyn = 1:no_dyn  
  ...
  %RTTrackerWrapper(Iref, I); % OLD
  RTTrackerWrapper(I, Iref); % OLD

  % Apply the estimated motion on the current image
  for n=1:nargin-3
      f_img(:,:,:,dyn,n) = RTTrackerWrapper(varargin{n});
  end
