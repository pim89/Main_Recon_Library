...
if ~isempty(regexp(dcmMR.ProtocolName,['\w*B1 calibration\w*']))
...
fprintf(['Acq date: %s @ %s ; PatiendID: %s (%s)\n',...
    '"%s" Imaging parameters @  %.2f T: %s cartesian %s %s , %i-echo (%s, %s, Fat-sat: ) \n',...
    'TE1/TE2/TE... = %s ms; TR = %.2f ms; FA= %i deg; BW = %.1f Hz; WFS =\n',...
    ...
    Spoil,...
    Sequence,nrEcho,...
    dcmMR.ScanningSequence,dcmMR.SequenceVariant,...
    data.TE,data.TR,dcmMR.FlipAngle,dcmMR.PixelBandwidth,...
